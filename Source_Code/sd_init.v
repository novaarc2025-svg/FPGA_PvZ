...
// File name:           sd_init
// Created by:          正点原子
// Created date:        2025年10月15日16:41:00
// Version:             V1.0
...
always @(posedge clk_ref or negedge rst_n) begin
    if(!rst_n) begin
        ...
        if(div_cnt == DIV_FREQ/2 - 8'd1) begin
            div_clk <= ~div_clk;
            div_cnt <= 8'd0;
        end
        else    
        ...
        case(cur_state)
            ...
            st_wait_cmd0 : begin                          
                ...
                if(res_en)                                //SD卡返回响应信号
                    //接收完成之后再拉高,进入SPI模式                     
                    sd_cs <= 1'b1;                                      
                over_time_cnt <= over_time_cnt + 16'd1;    //超时计数器开始计数
                //SD卡响应超时,重新发送软件复位命令
                if(over_time_cnt == OVER_TIME_NUM - 16'd1)
                    over_time_en <= 1'b1; 
                if(over_time_en)
                    over_time_cnt <= 16'd0;                                        
            end                                           
